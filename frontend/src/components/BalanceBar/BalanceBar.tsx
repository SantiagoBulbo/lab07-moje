...
const BalanceBar = () => {
  ...
  // Nasłuchuje na sygnał z reszty aplikacji i odświeża bilans
  useEffect(() => {
    if (!isAuthenticated) return;
    ...
    window.addEventListener("refreshGlobalBalance", handleGlobalRefresh);
    return () => window.removeEventListener("refreshGlobalBalance", handleGlobalRefresh);
  }, [isAuthenticated, range, refreshBalance]);
