...
const GroupMembersPage = ({ group, onBack }: Props) => {
  ...
  const applyMembersResults = useCallback(
      ([membersResult, debtsResult]: Awaited<
          ReturnType<typeof loadMembersData>
      >) => {
        setErrorMessage("");
        ...
        if (membersResult.status === "fulfilled") {
          ...
          setMembers([]);
          setErrorMessage(
              (current) => current || "Nie udało się pobrać członków grupy.",
          );
        }
        ...
        if (debtsResult.status === "fulfilled") {
          ...
          setDebts([]);
          setErrorMessage(
              (current) => current || "Nie udało się pobrać długów grupy.",
          );
        }
      },
      [],
  ...
  const refreshMembers = useCallback(async () => {
    ...
    // 🔥 TO JEST ZAPALNIK! Odświeża bilans na samej górze strony bez klikania F5
    window.dispatchEvent(new Event("refreshGlobalBalance"));
  }, [applyMembersResults, loadMembersData]);

  // Nasłuchuje na refreshGroupMembers z WebSocket
  useEffect(() => {
    window.addEventListener("refreshGroupMembers", refreshMembers);
    return () => window.removeEventListener("refreshGroupMembers", refreshMembers);
  }, [refreshMembers]);
